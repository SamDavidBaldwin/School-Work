...
class Parser(object):
    ...
    def writeSubroutine(self):
        self.output += "<symbol> " + self.value + " </symbol>\n"
        self.index += 1
        while self.allTokens[self.index] == "static" or self.allTokens[self.index] == "function":
            if self.allTokens[self.index] == "static":
                self.staticVarDec()
            if self.allTokens[self.index] == "function":
