...
void *SumExp(void *args_ptr) {
    ...
    int timetocomplete = end_time.tv_usec - start_time.tv_usec;
    printf("The time to complete Thread %d was %d microseconds\n", myargs_ptr->tid, timetocomplete);
    char buf[50];
    struct timeval return_time; 
    gettimeofday(&return_time, NULL);
    printf("The current time is: %s\n", ctime_r(&return_time.tv_sec, buf));
...
int main (int argc, char *argv[]) {
    ...
    gettimeofday(&end_time, NULL);
    int timetocomplete = end_time.tv_usec - start_time.tv_usec;
    printf("The time to complete was: %d microseconds\n", timetocomplete);
