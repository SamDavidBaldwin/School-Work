...
void process(char *root)
{
  ...
  int largestSize = 0;
  double runningSizeSum = 0.0;
  int numOfRegFiles = 0;
  int numOfDir = 0;
  struct stat file_info;
  time_t recentlyModified = 0;
  time_t leastRecentlyModified = 2147483647;
  ...
  char rmodname[MAXLENGTH] = "";
    ...
    fstat(fd, &file_info);
    if (file_info.st_size < smallestSize){
        smallestSize = file_info.st_size;
    }
    if(file_info.st_size > largestSize){
        largestSize = file_info.st_size;
    }
    runningSizeSum += file_info.st_size;

    switch(file_info.st_mode & S_IFMT){
        case S_IFREG: numOfRegFiles++; break;
        case S_IFDIR: numOfDir++; break;
        default: break;
    }

    if(file_info.st_mtime < leastRecentlyModified){
        leastRecentlyModified = file_info.st_mtime;
        strcpy(modname, dname);
    }
    else if(file_info.st_mtime > recentlyModified){
        recentlyModified = file_info.st_mtime;
    ...
    printf(" size of smallest file: %d, largest file: %d, average file: %f\n", smallestSize, largestSize, runningSizeSum/(double)numOfFiles);
