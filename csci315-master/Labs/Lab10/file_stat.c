...
int main(int argc, char *argv[]) {
  ...
  if (fstat(fd, &file_info) == 0) {
    ...
    getpwuid_r(fs_info.f_fsid, &pwd, buf, bufsize, &result);

    struct group grp;
    struct group *resultgrp;
    char*buf2 = malloc(bufsize);

    getgrgid_r(file_info.st_gid, &grp, buf2, bufsize, &resultgrp); 
    ...
    char asctime_str2[35];
    char asctime_str3[35];
    localtime_r(&(file_info.st_atime), &time);
    asctime_r(&time, asctime_str2);
    printf(" time of last access: %s\n", asctime_str2);

    localtime_r(&(file_info.st_ctime), &time);
    asctime_r(&time, asctime_str3);
