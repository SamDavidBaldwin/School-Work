	...
	*/
	...
	if (bytes_recv > 0){
		...
		if (strcmp(method, "GET")== 0){		
		    char filepath[MAX_LENG + 1];
            filepath[0] = '.';
            filepath[1] = '\0';

            strncat(filepath, path, MAX_LENG);

            int file_size = get_file_len(filepath);
            printf("%d",file_size);
            char response[MAX_LENG+1];
            if(file_size <= 0){
                snprintf(response, MAX_LENG, "HTTP/1.1 200 OK\r\nContent-Length:0\r\n\r\n");
            }
            snprintf(response, MAX_LENG, "HTTP/1.1 200 OK\r\nContent-Length:%d\r\n\r\n", file_size);
    
            if(writen(tp->sd, &response, strlen(response)) != strlen(response)){
                error("write error");
            }
            send_file(tp->sd, filepath);
